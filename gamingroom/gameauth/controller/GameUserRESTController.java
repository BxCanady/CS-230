...
public class GameUserRESTController {
    ...
    //  Add @RolesAllowed for USER role (Only authenticated users with USER role can access this)
    @RolesAllowed("USER")
    @GET
    @Path("/{id}")
    ...
    //  Add @RolesAllowed for ADMIN role (Only ADMIN users can create a new game user)
    @RolesAllowed("ADMIN")
    @POST
